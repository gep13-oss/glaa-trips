...
namespace AalgTrips.UnitTests
{
    ...
    /// <c>GLAA_TRIPS_BLOB_CONNECTION</c> environment variable — a full storage
    ...
    ///   set GLAA_TRIPS_BLOB_CONNECTION=UseDevelopmentStorage=true
    /// </code>
    ...
    public sealed class AzureBlobPhotoStoreTests : PhotoStoreContractTests
    {
        ...
        {
            _connectionString = Environment.GetEnvironmentVariable("GLAA_TRIPS_BLOB_CONNECTION");
        }
        ...
        protected override IPhotoStore CreateStore()
        {
            ...
            {
                Assert.Ignore("Set GLAA_TRIPS_BLOB_CONNECTION (e.g. to a running Azurite) to run the Azure Blob store contract tests.");
            }
